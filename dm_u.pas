...
type
  ...
  public
  pathUpdate,ftpuser,ftppass,fileNameUpdate:String;
  verDB:integer;
...
if ADO.Connected then begin
   ...
   try
    fileNameUpdate:=Trim(Q1.Fields[4].AsString);
   except
    fileNameUpdate:='';
   end;
...
main.ATXShowMessage('Нет соединения с сервером!');
res:=false;
end;
end;
result:=res;
...
begin
   if (E.Message='Row cannot be located for updating. Some values may have been changed since it was last read') then
    begin
    Main.ATXMessageDlg('Запись была изменена или удалена другим пользователем.',1,mtInformation);
      with Main.ActiveMDIChild as Tfrmavtocard do
...
Main.ATXMessageDlg('Ошибка сети. Связь с сервером прервана'+#13+'Необходимо перезапустить программу. Не сохраненные данные будут потеряны.',1,mtError)
   else
...
begin
   if (E.Message='Row cannot be located for updating. Some values may have been changed since it was last read') then
    begin
    Main.ATXShowMessage('Запись была изменена другим пользователем.');
