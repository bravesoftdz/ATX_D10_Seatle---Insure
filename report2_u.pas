...
{только с полисами, все машины, без полисов}
howselect:=0;
...
if what=1 then begin
  Caption:='Отчет - Расчет полиса';
  d1L.Caption:='Окончание полиса с ';
  d1L.Left:=8;
  LbB_Search.Caption:='Создать отчет';
end;
if what=3 then begin
  Caption:='Отчет - Не заполненные полиса';
...
if what=2 then begin
  Caption:='Расчет страховки и заполнение';
  d1L.Caption:='Окончание полиса с ';
  d1R.Date:=Now;
    d2R.Date:=Now;
    LbB_Search.Caption:='Расчитать';
  d1L.Left:=8;
...
if what=4 then begin
  Caption:='Отчет - Прогноз расчето полиса';
  d1L.Caption:='Окончание полиса с ';
  d1L.Left:=8;
  d2L.Visible:=false;
  d2R.Visible:=false;
