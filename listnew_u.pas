...
uses
  ...
  Dialogs, Grids, LbButton, StdCtrls, rxPlacemnt, DB, ExtCtrls;
...
if dm.StoredProc1.Fields[0].AsInteger>0 then inc(r);
  ...
  Main.ATXShowMessage('Добавлено '+IntToStr(r)+' тран.средств.');
...
begin
  ...
  begin
    ...
    if NewCheckBox <> nil then
    begin
      Rect := StringGrid1.CellRect(0, i); // получаем размер ячейки для чекбокса
      NewCheckBox.Left := Rect.Left + 2;
...
set_checkbox_alignment;
 ...
 if i=0 then begin
  ...
  Main.ATXShowMessage('Нет новых тран.средств. в БД!');
