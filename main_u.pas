...
Application.ProcessMessages;
    ...
    ATXShowMessage('В данном представлении нет записей!');
Application.ProcessMessages;
Application.ProcessMessages;
frmLoadData.Close;
end;
...
if not dm.CheckingUser(frmauth.edit_user.Text,frmauth.edit_pwd.Text) then
  begin
    ATXMessageDlg('У вас нет прав доступа к базе данных ! Обратитесь к администратору !'+#13+#13+#13,1,mtError);
    Application.Terminate;
  end
  else begin
      //доступ получен
//UserRole:=1;//admin
//UserRole:=2;//тех отдел
//UserRole:=3;//Штатка
//UserRole:=4;//User
...
case what of
1: begin frxReport1.ReportOptions.Name := 'Гарнизон/Область';
end;
2: begin frxReport1.ReportOptions.Name:='Управление';
end;
3:begin frxReport1.ReportOptions.Name:='Отделы';
end;
4:begin frxReport1.ReportOptions.Name:='Под-отделы';
end;
5:begin frxReport1.ReportOptions.Name:='Службы';
end;
6:begin frxReport1.ReportOptions.Name:='Марка автотранспорта';
end;
7:begin frxReport1.ReportOptions.Name:='Источник финансирования';
end;
8:begin frxReport1.ReportOptions.Name:='Тип кузова';
end;
9:begin frxReport1.ReportOptions.Name:='Тех. состояние';
end;
10:begin frxReport1.ReportOptions.Name:='Группа эксплуатации';
end;
11:begin frxReport1.ReportOptions.Name:='Место дислокации';
end;
15:begin frxReport1.ReportOptions.Name:='Страховые компании';
end;
16:begin frxReport1.ReportOptions.Name:='Модели';
frxDBDataSetLibr.DataSource:=dm.ds_libr2;
end;

17:begin frxReport1.ReportOptions.Name:='Категория ТС';
end;
18:begin frxReport1.ReportOptions.Name:='Под служба';
end;
22,23,24,25,26:begin frxReport1.ReportOptions.Name:='Справочник';
end;
...
frxReport1.ReportOptions.Name:='Тех.характеристики';
whatReport:=9;
...
if what=2 {расчет по страховке} then
begin
     Application.ProcessMessages;
     InsertEmptyInsures();
     frmLoadData.Close;
...
case what of
1:frxReport1.ReportOptions.Name:='Расчет полиса';
3:frxReport1.ReportOptions.Name:='Не заполненные полиса';
4:frxReport1.ReportOptions.Name:='Прогноз расчета полиса';
end;//case
...
dm.CommitTR;
       ATXShowMessage('Добавлено '+IntToStr(countRec)+' страховок, на сумму: '+FloatToStr(sumIns)+' руб.');
except
on E : Exception do
begin
dm.RollBTR;
...
on E : Exception do     begin
            ATXMessageDlg('Что-то пошло не так! '+E.Message,1,mtError);
            Application.ProcessMessages;
          end;
end;
...
begin
  ATXShowMessage('У Вас нет прав на выполнения данной команды!');
  Exit;
end;

  if Main.ATXMessageDlg('Вы уверены, что хотите Удалить все незаполненные полиса?',
...
dm.CommitTR;
       ATXShowMessage('Удалено '+IntToStr(dm.StoredProc_osago.Parameters[1].Value)+' страховок');
except
on E : Exception do
begin
dm.RollBTR;
...
1,2,4:begin
  ...
  Value := 'Предварительный расчет по страховкам на '+str+' '+IntToStr(myYear)+' года.';
  end;
...
3:begin
 if VarName ='report_title' then
  begin
  Value := 'Не заполненные полиса';//+str+' '+IntToStr(myYear)+' года.';
  end;
end;
9:begin
...
str:='Отстой от '+Tfrmavtocard(ActiveMDIChild).date_state.Text;
if Tfrmavtocard(ActiveMDIChild).out_account_box.Checked then
str:='Снятие с учета от '+Tfrmavtocard(ActiveMDIChild).date_out_account.Text;
if Tfrmavtocard(ActiveMDIChild).life_box.Checked then
str:='Списан от '+Tfrmavtocard(ActiveMDIChild).date_life.Text;
Value:=str;
end;
...
if VarName='libr_name' then
 Value:='Справочник по горнизонам';
end;
102:begin
if VarName='libr_name' then
 Value:='Справочник по управлениям';
end;
103:begin
if VarName='libr_name' then
 Value:='Справочник по отделам';
end;
104:begin
if VarName='libr_name' then
 Value:='Справочник по под-отделам';
end;
105:begin
if VarName='libr_name' then
 Value:='Справочник по службам';
end;
106:begin
if VarName='libr_name' then
 Value:='Справочник по марке автотранспорта';
end;
107:begin
if VarName='libr_name' then
 Value:='Справочник по источнику финансирования';
end;
108:begin
if VarName='libr_name' then
 Value:='Справочник по типу кузова';
end;
109:begin
if VarName='libr_name' then
 Value:='Справочник по тех. состоянию';
end;
110:begin
if VarName='libr_name' then
 Value:='Справочник по группе эксплуатации';
end;
111:begin
if VarName='libr_name' then
 Value:='Справочник по месту дислокации';
end;
112:begin
if VarName='libr_name' then
 Value:='Справочник по марке ГСМ';
end;
113:begin
if VarName='libr_name' then
 Value:='Справочник по виду ремонта';
end;
114:begin
if VarName='libr_name' then
 Value:='Справочник по месту ремонта';
end;
115:begin
if VarName='libr_name' then
 Value:='Справочник по страховым компаниям';
end;
116:begin
if VarName='libr_name' then
 Value:='Справочник по '+dm.tMark_avt.Fields[1].AsString;
end;

117:begin
if VarName='libr_name' then
 Value:='Справочник по категориям ТС';
end;
118:begin
if VarName='libr_name' then
 Value:='Справочник по под службам';
end;
122:begin
if VarName='libr_name' then
 Value:='Справочник по Коэффициенту КО';
end;
123:begin
if VarName='libr_name' then
 Value:='Справочник по Территории использования (КТ)';
end;
124:begin
if VarName='libr_name' then
 Value:='Справочник по Периоду использования (КС)';
end;
125:begin
if VarName='libr_name' then
 Value:='Справочник по Мощности двигателя (КМ)';
end;
126:begin
if VarName='libr_name' then
 Value:='Справочник по Базовым тарифам';
end;
