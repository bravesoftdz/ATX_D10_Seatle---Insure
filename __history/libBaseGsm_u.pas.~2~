...
DBGrid.Columns.Add;
DBGrid.Columns[0].Title.Caption:='Ќорма';
...
begin
  if Main.ATXMessageDlg('¬ы уверены, что хотите удалить запись: "'+dm.tBaseNormGsm.Fields[4].AsString+'" ?'+#13+
  '”даление приведет к удалению всего автотранспорта принадлежащего данной записи!',
    2,mtWarning) = mrYes then
    try
        dm.tBaseNormGsm.Delete;
    except
        Main.ATXMessageDlg('Ќе возможно удалить запись!',1,mtError);
    end;
